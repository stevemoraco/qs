...
const FEATURES = [
  {
    ...
    desc: "NIST FIPS 203 finalized standard. Quantum-safe key encapsulation designed to resist known quantum attacks, including Shor-style attacks against classical public-key cryptography.",
  },
  {
    icon: <Shield className="w-6 h-6" />,
    title: "ML-DSA-87 Message Signing",
    desc: "NIST FIPS 204 digital signatures. Message packages are signed and verified so tampering can be rejected before plaintext is shown.",
  },
  {
    icon: <Lock className="w-6 h-6" />,
    title: "AES-256-GCM Symmetric Layer",
    desc: "256-bit authenticated encryption for message payloads, paired with post-quantum key wrapping and signatures for the message package.",
  },
  {
    icon: <Eye className="w-6 h-6" />,
    title: "On-Device Camera Detection",
    desc: "When enabled in chat, an on-device detector uses the selfie camera to look for nearby recording devices and can lock sensitive content when it detects one.",
  },
  {
...
const HERO_FEATURES = [
  ...
  { icon: <ScanFace className="w-4 h-4" />, label: "Passkeys protect account access on supported devices" },
  { icon: <MousePointerClick className="w-4 h-4" />, label: "Hold-to-reveal messages that rehide on release" },
  { icon: <Server className="w-4 h-4" />, label: "Experimental quorum decay uses server-side time attestations" },
...
const DECAY_TIMELINE = [
  {
    ...
    copy: "Sixteen chunks are still readable while the message remains within its expiry window. The room warns that quorum time is almost out.",
  ...
  },
  {
    ...
    copy: "A copied key, fake server clock, or rolled-back phone clock is less useful when expiry follows server-side quorum time instead of only one device clock.",
  ...
  },
  {
    ...
    copy: "All sixteen chunks are now shown as decayed. The implementation is experimental and depends on correct key purge and attestation handling.",
...
const PROTOCOL_COMPARISON = [
  ...
  },
  {
    ...
    quantumShield: "When you send a message, the app gives it a fresh key and can blur the exact send time with random delivery fuzz. In experimental decay rooms, messages use server-side quorum time checks, so expiry does not depend only on one phone clock. The server carries the package but should not have plaintext or private identity keys; check the source code and verify the key path.",
  ...
  },
  {
    ...
    quantumShield: "You choose whether the timer starts when a message is first viewed or immediately when sent. In standard decay, clients purge local message keys and the API wipes wrapped key envelopes after expiry. In experimental quorum decay, reveal depends on server-side time agreement rather than only a copied key, a rolled-back device clock, or one fake server clock. Leftover server ciphertext should be unreadable without a usable key path. Check the source code and verify the timed key purge.",
...
const LOCAL_DEVICE_EXPOSURE = [
  {
    ...
    stopped: [
      ...
      "Expired local message keys and server-side wrapped key envelopes are removed so old ciphertext should remain unreadable without a usable key path.",
    ...
    implicationPlain: "A bad actor has a narrower plaintext window to target because messages are revealed one at a time.",
    implication: "QuantumShield is designed so account access is protected by passkeys, plaintext appears only for the message currently being held or intentionally revealed, and normal chat history avoids broad plaintext storage. Check the source code and verify the storage and reveal paths.",
  },
  {
    name: "Signal",
    tone: "risk",
          ...
          <div id="quorum-decay" className="mt-10 border border-amber-500/35 bg-amber-500/5 p-5 md:p-6 scroll-mt-16">
            <div className="grid grid-cols-1 lg:grid-cols-[0.75fr_1.25fr] gap-6 items-start mb-6">
              <div>
                ...
                <p className="font-mono text-sm text-muted-foreground leading-relaxed">
                  Experimental quorum decay models a message as many protected pieces. At minute 14 it still reveals inside the expiry window. At minute 15 server-side quorum time expires. By minute 17, all 16 chunks are represented as unreadable after the key path is purged.
                </p>
              </div>
              <div className="border border-border/50 bg-background/70 p-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    { icon: <ScanFace className="w-4 h-4" />, label: "Deliberate reveal", copy: "Hold or tap gates the live plaintext." },
