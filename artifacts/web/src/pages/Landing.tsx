...
const PROTOCOL_COMPARISON = [
  ...
  },
  {
    ...
    icon: <Key className="w-5 h-5" />,
    quantumShieldLead: "Every message gets a fresh post-quantum wrapped key and fuzzed delivery timing.",
    ...
    quantumShield: "When you send a message, the app creates a fresh message key and wraps it to each recipient device with ML-KEM-1024. Delivery can be randomly delayed inside a room's fuzz window, so observers cannot rely on exact send timing alone. The server carries the package but is mathematically guaranteed not to have the key needed to read it; check the source code and verify the key path.",
  ...
  },
  {
    ...
    icon: <TimerOff className="w-5 h-5" />,
    quantumShieldLead: "Timed key envelopes expire into useless ciphertext.",
    ...
    quantumShield: "You choose whether the timer starts when a message is first viewed or immediately when sent. After expiry, clients purge local message keys and the API wipes the wrapped key envelopes used to recover them. Leftover server ciphertext is mathematically guaranteed to be unreadable noise without a usable key path. Check the source code and verify the timed key purge.",
