...
const PROTOCOL_COMPARISON = [
  ...
  },
  {
    ...
    icon: <Key className="w-5 h-5" />,
    quantumShieldLead: "Every message gets a fresh post-quantum wrapped key, fuzzed delivery timing, and an optional experimental decay tier.",
    ...
    quantumShield: "When you send a message, the app creates a fresh message key and wraps it to each recipient device with ML-KEM-1024. Delivery can be randomly delayed inside a room's fuzz window, so observers cannot rely on exact send timing alone. Rooms can also opt into the experimental quorum-decay tier, which labels the room for future multi-attestation expiry policy work while standard post-quantum encryption remains active today. The server carries the package but is mathematically guaranteed not to have the key needed to read it; check the source code and verify the key path.",
  ...
  },
  {
    ...
    icon: <TimerOff className="w-5 h-5" />,
    quantumShieldLead: "Standard decay purges keys; experimental decay marks rooms for quorum expiry.",
    ...
    quantumShield: "You choose whether the timer starts when a message is first viewed or immediately when sent. In standard decay, clients purge local message keys and the API wipes wrapped key envelopes after expiry. The experimental quorum-decay tier is an opt-in room policy for future multi-source time attestation and mesh-enforced expiry work; it is not the default. Leftover server ciphertext is mathematically guaranteed to be unreadable noise without a usable key path. Check the source code and verify the timed key purge.",
