...
/-- BANKER: a regulator-independent lower gap `c > 0` survives an error budget
`θ*c` with `θ < 1`. The transferred observable retains the explicit positive
floor `(1-θ)c`. No separate premise `0 ≤ θ` is logically needed: together with
`c > 0`, the absolute-error hypothesis is already impossible when `θ < 0`. -/
...
/-- CRITIC: even a strict error smaller than each local regulated gap does not
produce any regulator-independent positive floor if the local gap itself can tend
to zero. For every proposed `μ>0` there is a positive local gap and a strictly
positive transferred gap below `μ`. -/
...
/-- CLEANER: the pointwise transfer becomes uniform when the regulated gap has a
uniform floor and the error budget is measured against that same floor. This
packages the quantifier order needed in a continuum-limit edge. -/
theorem ym_uniform_gap_and_relative_error_give_uniform_floor
  ...
  exact (ym_relative_error_preserves_fraction_of_uniform_gap
    (g a) (m a) c θ hc hθ1 (hgap a) (herr a)).1
