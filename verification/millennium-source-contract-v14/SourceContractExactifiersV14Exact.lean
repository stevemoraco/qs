...
/-- An order-`r` homogeneous zero-propagation rule reduces the infinite
predicate family to its first `r` entries. -/
...
/-- Operator-valued version: the complete common kernel is exactly the common
kernel of the first recurrence window.  The recurrence coefficients themselves
stay outside this finite consumer; the source supplies the zero-propagation
identity `hstep`. -/
...
/-! ## Tight-set lattice and atomization -/

/-- A zero-slack (tight) set for an integer-valued slack function. -/
def Tight {α : Type*} (q : Set α → ℤ) (A : Set α) : Prop :=
  q A = 0

/-- Nonnegative submodular slack has tight sets closed under union and
intersection.  This is the abstract finite consumer of the matroid-rank
calculation `q(A)=r(S_A)-|A|`. -/
...
/-- A nonempty tight set minimal among nonempty tight subsets. -/
def MinimalNonemptyTight {α : Type*} (q : Set α → ℤ) (A : Set α) : Prop :=
  A.Nonempty ∧ Tight q A ∧
    ∀ ⦃B : Set α⦄, B.Nonempty → Tight q B → B ⊆ A → A ⊆ B

/-- Two minimal nonempty tight sets with nonempty intersection are equal. -/
...
/-- Distinct minimal nonempty tight sets are disjoint. -/
...
/-- Integer slack consumer: every nonempty zero-slack set receives at least one
repair hit, while already-positive slack is automatically at least one. -/
