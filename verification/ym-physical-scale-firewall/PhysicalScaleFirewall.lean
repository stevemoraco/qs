...
noncomputable def spacing (n : ℝ) : ℝ := 1 / ((n : ℝ) + 1)
...
theorem positive_step_correlation_zero (n k : ℕ) (hk : 0 < k) : corr n k = 0 := by simp [corr, Nat.ne_of_gt hk]
theorem geometric_clustering (q : ℝ) (hq : 0 ≤ q) : ∀ n k : ℕ, |corr n k| ≤ q ^ k := by
  ...
  ÷ have hkpos : 0 < k := Nat.pos_of_ne_zero hk
    ...
    simpa only [abs_zero] using (pow_nonneg hq k)
...
theorem high_mass_at_most_half_of_defect_budget
  ...
  have h : (K * a) * (2 * high) ≥ (K * a) * V := by
    calc
      ...
      _ = (K * a) * V := by ring
  by_contra hh
  have hvh : V < 2 * high := by linarith
  have hlt : (K * a) * V < (K * a) * (2 * high) :=
    mul_lt_mul_of_pos_left hvh hka
  exact (not_lt_of_ge h) hlt

theorem low_mass_positive_of_defect_budget
    {V low high D a K v : ℝ} (ha : 0 < a) (hK : 0 < K)
