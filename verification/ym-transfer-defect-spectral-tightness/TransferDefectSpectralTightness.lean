...
namespace Millennium.YangMills.TransferDefectSpectralTightness
...
theorem high_weight_costs_transfer_defect
    ...
    delta * (∑ i in high, weight i) ≤
      ∑ i in high, weight i * (1 - transfer i) := by
  rw [Finset.mul_sum]
...
theorem sector_defect_le_total_defect
    ...
    (∑ i in sector, weight i * (1 - transfer i)) ≤
      ∑ i, weight i * (1 - transfer i) := by
...
theorem high_spectral_weight_le_half
    ...
    (hbudget :
      (∑ i, weight i * (1 - transfer i)) ≤
        delta * ((∑ i, weight i) / 2)) :
    (∑ i in high, weight i) ≤ (∑ i, weight i) / 2 := by
  have hcost :
      delta * (∑ i in high, weight i) ≤
        ∑ i in high, weight i * (1 - transfer i) :=
...
end Millennium.YangMills.TransferDefectSpectralTightness
