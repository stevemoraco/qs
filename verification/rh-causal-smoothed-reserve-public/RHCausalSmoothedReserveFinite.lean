...
theorem critical_first_moment_cubic_iff
  ...
  constructor
  · intro h
    have hdiff : 0 ≤ 2 * s - A := by linarith
    have hAs : 0 ≤ A * s := mul_nonneg hA hs
    have hquad : 0 ≤ 4 * s ^ 2 + 2 * s * A + A ^ 2 := by
      nlinarith [sq_nonneg A, sq_nonneg s, hAs]
    have hprod : 0 ≤ (2 * s - A) * (4 * s ^ 2 + 2 * s * A + A ^ 2) :=
      mul_nonneg hdiff hquad
    nlinarith [hprod]
  · intro h
    by_contra hnot
    have hgt : 2 * s < A := lt_of_not_ge hnot
    have hdiff : 0 < A - 2 * s := by linarith
    have hApos : 0 < A := by linarith
    have hAA : 0 < A * A := mul_pos hApos hApos
    have hAs : 0 ≤ A * s := mul_nonneg (le_of_lt hApos) hs
    have hss : 0 ≤ s * s := mul_nonneg hs hs
    have hquad : 0 < A ^ 2 + 2 * A * s + 4 * s ^ 2 := by
      nlinarith
    have hprod : 0 < (A - 2 * s) * (A ^ 2 + 2 * A * s + 4 * s ^ 2) :=
      mul_pos hdiff hquad
    nlinarith [hprod]
