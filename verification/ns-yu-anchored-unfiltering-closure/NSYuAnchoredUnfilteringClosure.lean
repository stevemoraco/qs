...
step in the filtered-vorticity route: exact or asymptotic collinearity with a
possibly moving filtered anchor is preserved under coordinatewise limits, and
a nonzero limit anchor selects an actual limiting line.
...
/-- Exact collinearity is stronger than necessary: it is enough that the
anchored minor itself tends to zero. -/
theorem vanishing_minor_survives_limit
    (x y a b : ℕ → ℝ) (X Y A B : ℝ)
    (hx : Tendsto x atTop (𝓝 X))
    (hy : Tendsto y atTop (𝓝 Y))
    (ha : Tendsto a atTop (𝓝 A))
    (hb : Tendsto b atTop (𝓝 B))
    (hminor : Tendsto (fun n => x n * b n - y n * a n) atTop (𝓝 0)) :
    X * B - Y * A = 0 := by
  have hlim :
      Tendsto (fun n => x n * b n - y n * a n) atTop
        (𝓝 (X * B - Y * A)) :=
    (hx.mul hb).sub (hy.mul ha)
  exact tendsto_nhds_unique hlim hminor
...
/-- Vanishing angular/minor error on one diagonal sequence is enough; no exact
nested fixed-filter limit is required. -/
theorem anchored_vanishing_minors_survive_coordinatewise_limit
    (x1 x2 x3 a1 a2 a3 : ℕ → ℝ)
    (X1 X2 X3 A1 A2 A3 : ℝ)
    (hx1 : Tendsto x1 atTop (𝓝 X1))
    (hx2 : Tendsto x2 atTop (𝓝 X2))
    ...
    (ha2 : Tendsto a2 atTop (𝓝 A2))
    (ha3 : Tendsto a3 atTop (𝓝 A3))
    (h12 : Tendsto (fun n => x1 n * a2 n - x2 n * a1 n) atTop (𝓝 0))
    (h13 : Tendsto (fun n => x1 n * a3 n - x3 n * a1 n) atTop (𝓝 0))
    (h23 : Tendsto (fun n => x2 n * a3 n - x3 n * a2 n) atTop (𝓝 0)) :
    X1 * A2 - X2 * A1 = 0 ∧
      X1 * A3 - X3 * A1 = 0 ∧
      X2 * A3 - X3 * A2 = 0 := by
  constructor
  · exact vanishing_minor_survives_limit x1 x2 a1 a2 X1 X2 A1 A2
      hx1 hx2 ha1 ha2 h12
  constructor
  · exact vanishing_minor_survives_limit x1 x3 a1 a3 X1 X3 A1 A3
      hx1 hx3 ha1 ha3 h13
  · exact vanishing_minor_survives_limit x2 x3 a2 a3 X2 X3 A2 A3
      hx2 hx3 ha2 ha3 h23
...
/-- Complete exact anchored closure theorem. A sequence of filter-dependent
lines converges to one actual line whenever the filtered field and one filtered
anchor converge coordinatewise and the limit anchor is nonzero. -/
...
/-- Complete diagonal form. Approximate filtered line geometry whose three
anchored minors tend to zero already forces one actual limiting line. -/
theorem anchored_line_survives_vanishing_minors
    (x1 x2 x3 a1 a2 a3 : ℕ → ℝ)
    (X1 X2 X3 A1 A2 A3 : ℝ)
    (hx1 : Tendsto x1 atTop (𝓝 X1))
    (hx2 : Tendsto x2 atTop (𝓝 X2))
    ...
    (hanchor : A1 ≠ 0 ∨ A2 ≠ 0 ∨ A3 ≠ 0) :
    ∃ c : ℝ, X1 = c * A1 ∧ X2 = c * A2 ∧ X3 = c * A3 := by
  have hminors := anchored_vanishing_minors_survive_coordinatewise_limit
    x1 x2 x3 a1 a2 a3 X1 X2 X3 A1 A2 A3
    hx1 hx2 hx3 ha1 ha2 ha3 h12 h13 h23
  exact zero_minors_nonzero_anchor_gives_scalar
    X1 X2 X3 A1 A2 A3 hanchor hminors.1 hminors.2.1 hminors.2.2
