...
theorem evenDyadicW32Term (j : ℕ) :
  ...
  rw [evenDyadicW32Factor, evenDyadicWeight, ← mul_pow]
  norm_num

theorem halfGeometricSum (N : ℕ) :
    (∑ j in Finset.range N, ((1 : ℝ) / 2) ^ j) =
      2 * (1 - ((1 : ℝ) / 2) ^ N) := by
  induction N with
  | zero => norm_num
  | succ N ih =>
...
theorem halfGeometricSum_le_two (N : ℕ) :
    (∑ j in Finset.range N, ((1 : ℝ) / 2) ^ j) ≤ 2 := by
  rw [halfGeometricSum]
...
theorem evenDyadicWeight_has_uniform_W32_budget (N : ℕ) :
    (∑ j in Finset.range N,
      evenDyadicW32Factor j * evenDyadicWeight j) ≤ 2 := by
  simp_rw [evenDyadicW32Term]
  exact halfGeometricSum_le_two N

theorem sixteenthGeometricSum (N : ℕ) :
    (∑ j in Finset.range N, ((1 : ℝ) / 16) ^ j) =
...
theorem flatFarField_weighted_budget (N : ℕ) :
    (∑ j in Finset.range N, evenDyadicWeight j * (1 : ℝ)) ≤ 2 := by
  simp only [mul_one, evenDyadicWeight]
  rw [sixteenthGeometricSum]
  have hpow : 0 ≤ ((1 : ℝ) / 16) ^ N := by positivity
  nlinarith

theorem flatFarField_unweighted_sum (N : ℕ) :
    (∑ _j in Finset.range N, (1 : ℝ)) = (N : ℝ) := by
  simp

theorem weightedPacking_does_not_by_itself_give_unweightedClosure (N : ℕ) :
    (∑ j in Finset.range N,
      evenDyadicW32Factor j * evenDyadicWeight j) ≤ 2 ∧
    (∑ j in Finset.range N, evenDyadicWeight j * (1 : ℝ)) ≤ 2 ∧
    (∑ _j in Finset.range N, (1 : ℝ)) = (N : ℝ) := by
...
theorem criticalDirection_shellCost (j : ℕ) :
  ...
  rw [criticalDirectionGradientSq, criticalTimeShellLength, ← mul_pow]
  norm_num

theorem criticalDirection_budget_grows_linearly (N : ℕ) :
    (∑ j in Finset.range N,
      criticalDirectionGradientSq j * criticalTimeShellLength j) = (N : ℝ) := by
  simp_rw [criticalDirection_shellCost]
  simp
...
theorem improvedDirection_uniform_budget (N : ℕ) :
    (∑ j in Finset.range N,
      criticalDirectionGradientSq j * criticalTimeShellLength j * alignmentGainSq j) ≤ 2 := by
  simp_rw [improvedDirection_shellCost]
  exact halfGeometricSum_le_two N
...
theorem endpointShellFourthMass_sum (N : ℕ) :
    (∑ j in Finset.range N, endpointShellFourthMass j) = (N : ℝ) := by
  simp [endpointShellFourthMass]

theorem improvedShellFourthMass_uniform_budget (N : ℕ) :
    (∑ j in Finset.range N, improvedShellFourthMass j) ≤ 2 := by
  simp [improvedShellFourthMass, halfGeometricSum_le_two]
