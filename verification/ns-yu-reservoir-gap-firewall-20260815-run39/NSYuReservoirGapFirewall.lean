...
/-- No positive reservoir-gap constant follows from positivity alone.  For every
`Λ>0` there are positive `A,O` with `A < Λ O`.  Any useful reservoir gap must
therefore come from additional PDE/geometric nondegeneracy, not scalar signs. -/
